...
// Palet dokumen: SIAN/TEAL, sama dengan laporan individu (tema versi
// sebelumnya), dengan gaya visual mengikuti UI aplikasi: brutalism — bayangan
// pekat, garis tebal, dan bilah judul beraksen.
//
// Susunan dokumen:
...
// ─── Palet (sian/teal — mengikuti tema laporan individu) ───
const INK = "#0F172A";
...
// bawah, senada dengan tema dokumen.
...
function drawKop(doc: jsPDF, margin: number, pageW: number): number {
	...
	// Logo diperbesar (58 pt) dan DIPAKSA BERLATAR PUTIH. PNG beralpha juga
	...
	fittedCenterText(doc, "UNIVERSITAS MUHAMMADIYAH METRO", pageW / 2, 36, pageW - margin * 2 - 210, 13, 9);
	doc.setFont("helvetica", "bold");
	fittedCenterText(doc, "PROGRAM PASCASARJANA", pageW / 2, 49, pageW - margin * 2 - 210, 10.5, 8);
	fittedCenterText(
		doc,
	...
	// Garis kop diturunkan agar tidak mepet logo yang kini lebih besar
	// (logo berakhir di y = 74).
	drawHex(doc, INK);
...
function drawRahasiaBadge(doc: jsPDF, pageW: number, margin: number) {
	...
	doc.rect(x, 16, w, h, "F");
	fillHex(doc, ACCENT);
	doc.rect(x, 16, w, 2.6, "F");
	textHex(doc, WHITE);
	doc.setFont("helvetica", "bold");
	doc.setFontSize(9.4);
	txt(doc, "RAHASIA", x + w / 2, 30, { align: "center" });
	textHex(doc, INK);
}

/** Judul bagian: bilah tinta pekat beraksen sian + keterangan kanan. */
function sectionTitle(
	...
	fillHex(doc, ACCENT);
	doc.rect(margin, y, 5, 19, "F");
	textHex(doc, WHITE);
	doc.setFont("helvetica", "bold");
	fittedLeftText(doc, title, margin + 12, y + 13.5, innerW * 0.55, 10, 7.4);
	if (hint) {
		textHex(doc, AQUA);
		doc.setFont("helvetica", "bold");
		doc.setFontSize(6.6);
...
function drawKpiCard(
	...
	// Bilah aksen tipis di kepala kartu (gaya kartu pada UI aplikasi).
	fillHex(doc, ACCENT);
	doc.rect(x, y, w, 3, "F");
	textHex(doc, ACCENT_DEEP);
	doc.setFont("helvetica", "bold");
	fittedCenterText(doc, label.toUpperCase(), x + w / 2, y + 16, w - 14, 7.6, 5.2);
	textHex(doc, INK);
...
function drawBandChart(
	...
	doc.setFont("helvetica", "bold");
	doc.setFontSize(8.6);
	txt(doc, "SEBARAN KLASIFIKASI IQ KELOMPOK", margin + 12, yIn + 12.5);
	textHex(doc, AQUA);
	doc.setFontSize(6.6);
...
function drawNarrative(
	...
	lines.forEach((l, i) => {
		wrapped.push(...wrapText(doc, l, innerW - 28));
		if (i < lines.length - 1) wrapped.push("");
	});
	const boxH = 24 + wrapped.length * 9.8;

	brutBox(doc, margin, yIn, innerW, boxH, ACCENT_SOFT, 3.5);
	// Bilah aksen di tepi kiri (gaya callout pada UI aplikasi).
	fillHex(doc, ACCENT_DEEP);
	doc.rect(margin, yIn, 4, boxH, "F");
	textHex(doc, ACCENT_DEEP);
	doc.setFont("helvetica", "bold");
	doc.setFontSize(7.4);
	txt(doc, "BACAAN SINGKAT HASIL KELOMPOK", margin + 14, yIn + 15);
	textHex(doc, INK);
	doc.setFont("helvetica", "normal");
	doc.setFontSize(7.8);
	wrapped.forEach((line, i) => {
		txt(doc, line, margin + 14, yIn + 28 + i * 9.8);
	});
	return yIn + boxH;
}
...
function drawMethodology(
	...
	for (const n of notes) {
		wrapped.push(...wrapText(doc, `-  ${n}`, innerW - 30));
	}
	...
	doc.setFontSize(7.4);
	wrapped.forEach((line, i) => {
		txt(doc, line, margin + 14, yIn + 16 + i * 9.4);
	});
	return yIn + boxH + 14;
}
